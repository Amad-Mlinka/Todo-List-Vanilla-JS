...
$(async () => {
    ...
    todoList.todos.forEach((todo, i) => {
        var todoItem = $(`<div></div>`).addClass("todoItem");
        var todoCheck= $("<div></div>").addClass("todoCheck");
        var input=$("<input></input>").addClass("checkCircle").attr("type","checkbox").attr(todo.completed? "checked": "")
        var todoDescription=$("<div></div>").addClass("todoDescription").text(todo.title)

        todoCheck.append(input).append(todoDescription)
